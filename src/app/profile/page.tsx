...
const initialFarmerProfile = {
  name: 'Farmer Singh', // Default fallback
  ...
  bio: "A passionate farmer focused on integrating sustainable and modern agricultural practices to improve yield and soil health. Always eager to learn and connect with fellow farmers."
};
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    // Load user data from LocalStorage to sync with Header
    ...
    if (storedUser) {
      try {
        ...
        // If username IS the email, use it. Otherwise fallback to user.email or default.
        ...
        setProfile(prev => ({
          ...
          name: displayName,
          email: displayEmail,
          avatarUrl: `https://ui-avatars.com/api/?name=${displayName}&background=random&size=200`
        }));
        setFormData(prev => ({
          ...prev,
          name: displayName,
  ...
  const handleSave = () => {
    ...
    // Update LocalStorage so Header and next session see the changes
    const storedUser = localStorage.getItem('cropnavi_user');
    let updatedUser = {};
    if (storedUser) {
      try {
    ...
    updatedUser = {
      ...updatedUser,
      username: formData.name, // Update username to match Profile Name
      email: formData.email
    };

    localStorage.setItem('cropnavi_user', JSON.stringify(updatedUser));

    // Dispatch event to notify Header (if it was listening, but it mounts once)
    // For now, it will update on reload. 
